...
class AceAttorney(commands.Cog):
        ...
        for positionInQueue, render in enumerate(iterable=renderQueue, start=1):
            try:
                ...
                if render.getState() == State.RENDERED:
                    ...
                    outputFile = f"{self.data_folder}/{render.getOutputFilename()}"
                    fileSize = os.path.getsize(outputFile)
                    logging.info(f"{fileSize} File: {outputFile}")
                    if fileSize < render.getContext().channel.guild.filesize_limit:
                        await render.getContext().send(content=render.getContext().author.mention, file=discord.File(outputFile))
                        render.setState(State.DONE)
                        newFeedback = f"""
                        `Fetching messages... Done!`
                        `Your video is being generated... Done!`
                        ...
                        try:
                            ...
                            """
                            await render.updateFeedback(newFeedback)
                            with open(outputFile, 'rb') as videoFile:
                                files = {'files[]': (outputFile, videoFile)}
                ...
                if render.getState() == State.DONE:
                    self.clean(render.getMessages(), outputFile)
    ...
    async def queue(self, context):
        ...
        with open(filename, 'w+') as queue:
            ...
            for positionInQueue, render in enumerate(iterable=renderQueue):
                queue.write(f"\n#{positionInQueue:04}\n")
                try: queue.write(f"Requested by: {context.author.name}#{context.author.discriminator}\n")
                except: pass
    ...
    async def acerender(self, ctx, numberOfMessages: int = 0, music: str = 'pwr'):
        ...
        try:
            ...
            #discordMessages += await ctx.channel.history(limit=numberOfMessages, oldest_first=False, before=baseMessage).flatten()
